...
依存性逆転の原則を適用したことで変更に強くなった `Printer` クラス。

ここで，ソフトウェアの変更しやすさのメトリクスである結合度に着目してみます。
...
このクラスの外部モジュールへの結合は4箇所あることがわかります。
...
1つの `Printable` 型と，1つの `print_to_paper()` に依存していて，外部モジュールへの結合は2箇所だけです。

このように，リファクタリングによって「結合度」が下がる ＝　疎結合 なクラスになったことがわかります。
